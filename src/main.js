...
(function (root) {
  ...
  function resetCycle() {
    ...
    /* Каждое новое существо начинает на новой карте, в новом секторе: пока
       сменяли существо, корабль ушёл дальше. Сектор выброса капсулы уже
       записан в журнал. */
    mapId++;
    F.loadWorld(F.freshWorld({ w: NAV_W, h: NAV_W, seed: (Math.random() * 0xffffff) | 0, zoom: 1 }));
    LORE.nextSector();
    move = null; guide = null;
