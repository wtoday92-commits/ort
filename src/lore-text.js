...
(function (root) {
  ...
  var ship = {
    ...
    // работа на месте идёт всё хуже: корабль винит место, а не того, кто работает
    relocate_ineff: ['loss', 'fills', 'place', 'ship', 'leaves', 'place'],
  ...
  var logs = [
    ...
    /* Корабль ушёл с места, потому что работа там шла всё хуже. Он не ищет
       причину в существе: неэффективность для него — свойство места. */
    {
      id: 'r_relocate', color: 'white', w: 8, h: 3, priority: 2, when: { ineffMoves: 1 },
      sentences: [
        { text: ['РАБОТА', '⟨на-этом⟩', 'МЕСТО', '⟨идёт-хуже-чем-назначено⟩'], key: ['РА', 'БО', 'ТА'] },
        { text: ['ПОТЕРЯ', '⟨учитывается-за-каждый⟩', 'ЦИКЛ'], key: ['ПО', 'ТЕ', 'РЯ'] },
        { text: ['МЕСТО', '⟨признаётся⟩', 'НЕПРИГОДНЫЙ'], key: ['МЕС', 'ТО'] },
        { text: ['КОРАБЛЬ', '⟨перемещается-в-сторону⟩', 'СЛЕДУЮЩИЙ', 'МЕСТО'], key: null }
      ]
    },
